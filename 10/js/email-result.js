...
button.addEventListener("click", function() {
  ...
  if (email.value !== "") {
    username = email.value.split("@")[0];         // @ 기준으로 쪼개어 앞부분 저장
    half = username.length / 2;                    // username 의 길이를 반으로 나눔
    username = username.substring(0, (username.length - half));         // username 에서 반으로 나눈 위치까지만 추출
    domain = email.value.split("@")[1];            // @ 기준으로 쪼개어 뒷부분 저장
